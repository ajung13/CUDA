...
__global__ void TransformSOAwithConstantMemKernel( INOUT POINTS_SOA A, IN int m )
{
	...
	float eachIDReg, eachIDResult;
	for( i = 0; i < ELEM_PER_POINT; ++i )
	{
		eachIDResult = eachIDReg = A.elem[ i ][ id ];
		for( j = 2; j <= m; j++ )
		{
			eachIDResult += constantBuffer[j-1] * eachIDReg;
			//A.elem[ i ][ id ] = constantBuffer[ j - 1 ] * A.elem[ i ][ id ];
		}
		A.elem[ i ][ id ] = eachIDResult;
	}
}

__global__ void TransformAOSwithConstantAndSharedMemKernel( INOUT POINT_ELEMENT *A, IN int m )
{
	int row = blockIdx.y * blockDim.y + threadIdx.y;
	int col = blockIdx.x * blockDim.x + threadIdx.x;
	int id = gridDim.x*blockDim.x*row + col;
	...
	int threadIDperPoint = (threadIdx.y * blockDim.x + threadIdx.x) * ELEM_PER_POINT;

	for( int i = 0; i < ELEM_PER_POINT; ++i )
	{
		sharedBuffer[ threadIDperPoint + i ] = A[ id ].elem[ i ];
	}
	for( j = 2; j <= m; j++ )
	{
		for( int i = 0; i < ELEM_PER_POINT; ++i )
		{
			sharedBuffer[ threadIDperPoint + i ] += constantBuffer[ j - 1 ] * sharedBuffer[ threadIDperPoint + i ];
		}
	}
	for( int i = 0; i < ELEM_PER_POINT; ++i )
	{
		A[ id ].elem[ i ] = sharedBuffer[ threadIDperPoint + i ];
	}
}

__global__ void TransformSOAwithConstantAndSharedMemKernel( INOUT POINTS_SOA A, IN int m )
{
	int row = blockIdx.y * blockDim.y + threadIdx.y;
	int col = blockIdx.x * blockDim.x + threadIdx.x;
	int id = gridDim.x*blockDim.x*row + col;
	...
	for( i = 0; i < ELEM_PER_POINT; ++i )
	{
		sharedBuffer[ threadID ] = A.elem[ i ][ id ];
		//tmp = A.elem[ i ][ id ];
		for( j = 2; j <= m; j++ )
		{
			sharedBuffer[ threadID ] += constantBuffer[ j - 1 ] * sharedBuffer[ threadID ];
			//tmp += constantBuffer[ j - 1 ] * tmp;
		}
		A.elem[ i ][ id ] = sharedBuffer[ threadID ];
		//A.elem[ i ][ id ] = tmp;
	}
}
...
void transform_points_SOA( INOUT POINTS_SOA p_SOA, IN int n_points, IN int m )
{
	...
	for( int i = 0; i < ELEM_PER_POINT; ++i )
	{
		cudaMalloc( &d_pSOA.elem[i], size ); cudaMemcpy( d_pSOA.elem[i], p_SOA.elem[i], size, cudaMemcpyHostToDevice );
	}
...
void transform_points_AOS_with_constant_shared( INOUT POINT_ELEMENT *p_AOS, IN int n_points, IN int m )
{
	cudaEvent_t start, stop;
	cudaEventCreate( &start );
	cudaEventCreate( &stop );
	...
	dim3 dimBlock( SHARED_AOS_BLOCK_WIDTH, SHARED_AOS_BLOCK_HEIGHT );
	dim3 dimGrid( ARRAY_2D_WIDTH / dimBlock.x, ARRAY_2D_HEIGHT / dimBlock.y );

	cudaEventRecord( start, 0 );
	TransformAOSwithConstantAndSharedMemKernel << <dimGrid, dimBlock, sizeof( float )* SHARED_AOS_BLOCK_WIDTH * SHARED_AOS_BLOCK_HEIGHT * ELEM_PER_POINT >> >( d_pAOS, m );
	//TransformAOSwithConstantMemKernel << <dimGrid, dimBlock >> >( d_pAOS, m );
...
void transform_points_SOA_with_constant_shared( INOUT POINTS_SOA p_SOA, IN int n_points, IN int m )
{
	cudaEvent_t start, stop;
	cudaEventCreate( &start );
	cudaEventCreate( &stop );
	...
	dim3 dimGrid( ARRAY_2D_WIDTH / dimBlock.x, ARRAY_2D_HEIGHT / dimBlock.y );
	cudaEventRecord( start, 0 );
	TransformSOAwithConstantAndSharedMemKernel << <dimGrid, dimBlock, sizeof(float) * BLOCK_WIDTH * BLOCK_HEIGHT >> >( d_pSOA, m );
...
int main(void){
	...
	transform_points_AOS_with_constant_shared( Points_AOS, n_points, cutoff );
	transform_points_SOA_with_constant_shared( Points_SOA, n_points, cutoff );
